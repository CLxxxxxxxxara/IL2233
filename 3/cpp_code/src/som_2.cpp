...
using namespace std;
void readDataFromFile(const std::string& filename, double* data, int* labels, int n_samples, int m_features) {
    std::ifstream file2023("D:/KTH/IL2233_P4/lab3/cpp_code/src/iris.txt",ios::in);
    //file2023.open("D:\\KTHIL2233_P4\\lab3\\cpp_code\\src\\iris.txt");//std::ios::in
    if (file2023.is_open()){
   //  if(1){
        std::cout<<"hello"<<std::endl;
        std::string line;
        int sample_idx = 0;
        while (std::getline(file2023, line)) {
            std::stringstream ss(line);
            std::string token;
            int feature_idx = 0;
            while (std::getline(ss, token, ',')) {
                if (feature_idx < m_features) {
                    data[sample_idx * m_features + feature_idx] = std::stod(token);
                    feature_idx++;
                } else {
                    labels[sample_idx] = std::stoi(token);
                }
            }
            std::cout <<labels[sample_idx]<<std::endl;
            sample_idx++;
        }
        file2023.close();
    } else {
...
int main()
{
    ...
    int n_samples = 150;  // Number of samples
    int m_features = 4;   // Number of features

    // Read the dataset from a file
    const std::string filename = "iris.data";
    double* data = new double[n_samples * m_features];
    int* true_assignment = new int[n_samples];
    ...
    double randIndex = calculateRandIndex(som_cluster_assignment,true_assignment,n_samples);
